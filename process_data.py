...
def process_printer_data(input_file, output_file):
    ...
    for print_group in prints_to_process:
        ...
        metrics['Accel_print'] = group['accel_print'].mean() if group['accel_print'].notna().any() else 0.0
        metrics['Accel_retract'] = group['accel_retract'].mean() if group['accel_retract'].notna().any() else 0.0
        metrics['Accel_travel'] = group['accel_travel'].mean() if group['accel_travel'].notna().any() else 0.0
        metrics['Média jerk_x'] = group['jerk_x'].mean() if group['jerk_x'].notna().any() else 0.0
        metrics['Média jerk_y'] = group['jerk_y'].mean() if group['jerk_y'].notna().any() else 0.0
